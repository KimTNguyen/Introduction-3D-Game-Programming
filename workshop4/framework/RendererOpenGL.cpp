...
// Modified: 7 Mar 2010 - Kim Nguyen - SCIS, Edith Cowan University.
...
{	
	...
	bool isFirstCamera = bigExcavator.getFirstCamera();
	...
	// This will give us a 3rd person camera:
	// set up camera coordinates and point the camera at (ExcavatorX,1,ExcavatorZ)
	if (!isFirstCamera)
	{
		gluLookAt(ExcavatorX+(cos((angleAroundY-90)*3.14/180)*35), 20, 
			      ExcavatorZ+(-sin((angleAroundY-90)*3.14/180)*35), ExcavatorX, 1, ExcavatorZ, 0, 1, 0);
	}
	...
	else
	{
		...
		glTranslatef(-ExcavatorX, -13, -ExcavatorZ);	
	}
...
void RendererOpenGL::initialise()
{
	...
	// load texture
	m_textureObject = TextureCreator::loadTexture("skyboxN.bmp");
