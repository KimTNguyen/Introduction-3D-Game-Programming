...
public:
	~Geometry()
	{
		glDeleteBuffers(1, &verBufObject);
		glDeleteBuffers(1, &triBufObject);
	}
	...
	{
		glGenBuffers(1, &verBufObject);
		glBindBuffer(GL_ARRAY_BUFFER, verBufObject);

		glBufferData(GL_ARRAY_BUFFER, m_vertexCoordinates.size()*sizeof(Vertex3),0, GL_STATIC_DRAW);
		Vertex3 *vertBuff = (Vertex3 *)glMapBuffer(GL_ARRAY_BUFFER,GL_WRITE_ONLY);

		for (unsigned int count = 0; count < m_vertexCoordinates.size(); ++count)
		{
			vertBuff[count] = m_vertexCoordinates[count];
		}

		glUnmapBuffer(GL_ARRAY_BUFFER);

		glGenBuffers(1, &triBufObject);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, triBufObject);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_triangleIndices.size()*3*sizeof(GLuint),0, GL_STATIC_DRAW);

		GLuint *triBuff = (GLuint *)glMapBuffer(GL_ARRAY_BUFFER,GL_WRITE_ONLY);

		for (unsigned int count = 0; count < m_triangleIndices.size(); ++count)
		{
			triBuff[count] = m_triangleIndices[count];
		}
	...
	void drawOpenGLVertexBufferObject()
	{
		...
		glVertexPointer(3, GL_FLOAT, 0, (float*) 0);
		
		glDrawElements(GL_TRIANGLES, m_vertexCoordinates.size(), GL_UNSIGNED_INT, 0);
