...
// Modified: 30 Feb 2010 - Kim Nguyen - SCIS, Edith Cowan University.
...
{	
	...
	float scoopAngle = bigExcavator.getScoopAngle();
	...
	glPushMatrix();
		glColor3f(1, 0, 0);
		glTranslatef(ExcavatorX, 0, ExcavatorZ);
		...
		glPushMatrix();
			...
			glPushMatrix();
				...
				glPushMatrix();
					glTranslatef(0, 0, -5.3/2);	// translate to the attachment point
					glRotatef(upperArmAngle, 1, 0, 0);

					glPushMatrix();
						glTranslatef(0 ,0 ,5.3/2);	// translate back to the center of the upper arm
						DrawCube(0.5, 0.5, 5.3);
					glPopMatrix();
					...
					glPushMatrix();
						...
						glPushMatrix();
							glTranslatef(0, 0, -1.9); // translate to the attachment point
							glRotatef(lowerArmAngle, 1, 0, 0);

							glPushMatrix();
								glTranslatef(0, 0, 1.9); // translate back to the center of the upper arm
								DrawCube(0.5, 0.5, 3.8);
							glPopMatrix();

							// draw the scoop
							glTranslatef(0, -0.85, 3.9);
							glPushMatrix();
								glTranslatef(0, 0.85, -0.1);
								glRotatef(scoopAngle, 1, 0, 0);
...
{
	glClearColor(0.5,0.5,0.5,0.0);						// select what colour the background is (here set to black)
